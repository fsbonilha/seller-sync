...
class SpreadsheetSplitter:
    ...
    def __init__(self, template_file: str, input_data_file: str, input_sheet_names: list, output_folder: str, id_column: str, filename_location: dict) -> None:
    ...
    def import_data(self) -> dict:
        ...
        # df = pd.read_excel(self.input_data_file, sheet_name=self.input_sheet_names[0], header=0)
        # df_skus = pd.read_excel(self.input_data_file, sheet_name=self.input_sheet_names[1], header=0)
        # df_contacts = pd.read_excel(self.input_data_file, sheet_name=self.input_sheet_names[2], header=1)
    ...
    def split_sellers(self) -> None:
        ...
        for merchant_id in merchant_list:
            ...
            # sp_df = self.df[self.df[self.id_column] == merchant_id]
            # sku_df = self.df_skus[self.df_skus[self.id_column] == merchant_id]
    ...
    def fill_cols(self, merchant_dfs: dict) -> None:
        ...
        for ws in self.input_sheet_names:
            ...
            for index, row in current_df.reset_index().iterrows():
                row_index = index + 2  # considering header and excel 1 based counting
                for ncol, col in enumerate(current_df.columns):
                    worksheet.cell(row=row_index, column=ncol + 1, value=row[col])
    ...
    def get_save_path(self, merchant_dfs) -> str:
        ...
        seller_name = filename_sheet[self.filename_location["col"]].iloc[0].replace(' ', '_')
    ...
    def get_contacts(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(contacts_df.values[1:], columns=contacts_df.iloc[0])
...
def send_multiple_emails(files, emails):
    assert len(files) == len(emails), "Len of File List is not equal Len of Email List"
    
    for idx, file in enumerate(files): 
        mail_man = Mailman(recipient=emails[idx],
                           subject = "Amazon FBA - Relatório de Performance",
                           body = '''
                           Olá, vendedor, aqui está o seu relatorio semanal de performance de vendas.
                           ''',
...
def confirm_send_email():
    return (input("Please check files and type \"send\" to send emails: ") == "send")
...
def main():
    ...
    if confirm_send_email():
        send_multiple_emails(deal_splitter.files, deal_splitter.contacts['email'])
